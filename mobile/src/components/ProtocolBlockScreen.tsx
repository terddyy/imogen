...
const RELEASES_URL = 'https://github.com/terddyy/ImogenAI/releases'
...
export function ProtocolBlockScreen({ verdict }: Props) {
  ...
  const title = isMobileTooOld ? 'Update Imogen Mobile' : 'Update Imogen on your computer'
  const body = isMobileTooOld
    ? `This desktop needs a newer Imogen Mobile app. Update Imogen Mobile from ${mobileUpdateTarget.storeName}, then try this host again.`
    : 'This paired desktop app is too old for your current Imogen Mobile app. Update Imogen on your computer, then try this host again.'
  const recoveryNote =
