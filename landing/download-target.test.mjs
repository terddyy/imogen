...
assert.equal(asset({ platform: 'Win32' }), 'orca-windows-setup.exe')
assert.equal(asset({ platform: 'macOS', architecture: 'arm' }), 'orca-macos-arm64.dmg')
assert.equal(asset({ platform: 'MacIntel' }), 'orca-macos-x64.dmg')
