...
function probeNotificationDelivery(): Promise<NotificationDeliveryProbeResult> {
  ...
  const probe = new Notification({
    title: 'Imogen notifications are on',
    body: 'Imogen will alert you when agents finish or terminals need attention.',
    silent: true
  })
...
export function triggerStartupNotificationRegistration(store: Store): void {
  ...
  const notification = new Notification({
    title: 'Imogen is ready to notify you',
    body: 'Allow notifications so Imogen can alert you when agents finish or terminals need attention.'
  })
